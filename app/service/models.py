...
class UserProfile(BaseModel):
    ...
    live_metro_station: list[str] = Field(description="Станция метро, на которой ты живешь")
    study_metro_station: list[str] = Field(description="Станция метро, на которой ты учишься/работаешь")
    year_of_admission: int = Field(description="Год поступления в СтС")
    has_driver_license: UserDriverLicense = Field(description="Есть ли у тебя водительские права и/или машина?")
    date_of_birth: date_type = Field(description="Дата Рождения")
    has_printer: bool = Field(description="Если ли у тебя принтер?")
    can_host_night: bool = Field(description="Можем ли мы проводить ночь креатива/ночь оформления у тебя дома?")
